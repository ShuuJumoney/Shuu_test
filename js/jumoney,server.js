	...
	*/
	...
	async function getJumoney(data, npc) {
		...
		for (const key of items) {
			...
			table += `<img src="${url}" alt="${item_nm}" class="api-img">`;
			...
			table += `<img class="mabibase-img" data-index="${count}" item-name="${item_nm}" onerror="this.src='./cute.png'">`;
		...
		//src="${await createJumoneyImage(item_nm, colorArray)}"
	    ...
	    const imagePromises = items.map(async (key, index) => {
	        ...
	        // Jumoney 이미지 생성
	        const imgUrl = await createJumoneyImage(item_nm, color);
	...
	async function createNewItem(qValue, itemGroup) {
	    ...
	    let html = `
	        ...
	        <img src="${url}" alt="${firstKey}" class="api-img">
	        <img class="mabibase-img" onerror="this.src='./cute.png'" data-index="${qValue}" item-name="${firstKey}">
	    ...
	    const imgUrl = await createJumoneyImage(firstKey, Object.values(color).map(entry => entry.hex));
    ...
    async function fetchNpcData(npc, server, channel) {
        ...
        */
        ...
        if (!isResetNeeded() && dataCache[cacheKey] ) {
			//간혹 리셋 되었는데 캐시된 데이터 사용한다는 로그가 뜨며 리스트 생성 안하는 증상 방지
			if( dataCache[cacheKey] ) { //데이터가 없다면 다시 불러오기
	            console.log(`캐시된 데이터 사용: ${cacheKey}`);
	            document.getElementById("curCallState").innerText = `캐시된 데이터 사용: ${cacheKey}`;
        ...
        try {
			...
			*/
            ...
            console.log(`API 호출: ${cacheKey}`);
             document.getElementById("curCallState").innerText = `API 호출: ${cacheKey}`;
			const url = `http://shuua.dothome.co.kr/nexon_cache.php?npc_name=${npc}&server_name=${server}&channel=${channel}`;
			const response = await fetch(url);
			
			if (!response.ok) {
	            const errorData = await response.json();
	            console.error(`API 오류: ${errorData.error}`);
	            alert(`API 호출 실패: ${errorData.error}`);
	            return null;
	        }
	        ...
	        if (!data || data.length === 0) {
	            console.warn(`유효하지 않은 데이터: ${cacheKey}`);
	            return null;
	        }
			    ...
			    if (imageCache.has(cacheKey)) {
				    ...
				    if (itemData) {
				        ...
				        */
				        ...
				        imageUrl = await createJumoneyImage(itemName, colors);
