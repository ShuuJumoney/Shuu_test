...
document.addEventListener("DOMContentLoaded", function () {
	...
	async function getJumoney(data, npc) {
		...
		for (const key of items) {
			...
			table += `<img class="mabibase-img" data-index="${count}" onerror="this.src='./cute.png'">`;
	    ...
	    const imagePromises = items.map(async (key, index) => {
	        ...
	        const imgElement = document.querySelector(`.mabibase-img[data-index="${index}"]`);
	        if (imgElement) {
	...
	*/
	...
	async function createNewItem(qValue, itemGroup) {
	    ...
	    let html = `
	        ...
	        <img src="${mabibaseUrl}" class="mabibase-img" onerror="this.src='./cute.png'">
