...
document.addEventListener("DOMContentLoaded", function () {
	...
	*/
	...
	async function createItemImage(itemName, itemData) {
	    ...
	    const fallbackImage = "./cute.png"; // 실패 시 표시할 대체 이미지
	    imgElement.src = imageUrl;
	    imgElement.alt = itemName;
	    imgElement.classList.add("mabibase-img", "hidden"); // 초기에는 숨김
	
	    // 이미지 로드 성공 시 스피너 제거 및 이미지 표시
	    imgElement.onload = () => {
	    ...
	    imgElement.onerror = () => {
	        ...
	        spinner.style.display = "none"; // 스피너 숨김
	        imgElement.classList.remove("hidden"); // 이미지 표시
	        //console.warn(`이미지 로딩 실패: ${imageUrl}, 대체 이미지로 전환`);
	    };
